...
public class PopuJar extends PopupWindows implements OnDismissListener {
	...
	private int bgimg = 0;// 背景图片资料，为0则默认
	...
	public PopuJar(Context context, int bgImg) {
		this(context, VERTICAL, bgImg);
	}
	 ...
	 */
	public PopuJar(Context context, int orientation, int bgimg) {
		super(context);
	...
	public void setRootViewId(int id) {
		...
		if (bgimg != 0) {
			mScroller.setBackgroundResource(bgimg);// 设置背景图片
		}
	...
	public void addPopuItem(PopuItem action) {
		if (action == null || action.getTitle() == null || action.getTitle().length() == 0)//标题为空就不添加
			return;
		...
		View itemMain = container.findViewById(R.id.popup_item_main);
		...
		// View lineView = mInflater.inflate(R.layout.popup_line, null);
		// mTrack.addView(lineView,1);

		mChildPos++;
		mInsertPos++;

		// 最后一个，不显示分隔线
		if (action.isIslastitem()) {
			ImageView line = (ImageView) container.findViewById(R.id.popu_line);
