...
package com.earthman.app.zxing.view;
...
public final class ViewfinderView extends View {
	...
	public void onDraw(Canvas canvas) {
		...
		//Modify 此处修改为根据ViewFinderView的大小去确定扫描框的大小及位置，而非根据屏幕的大小确定
		//Rect frame = CameraManager.get().getFramingRect(new Point(width, height));
		if (frame == null) {
			return;
		}
		...
		if (resultBitmap != null) {
			...
			String text = getResources().getString(R.string.scan_des);
			float textWidth = paint.measureText(text);
			canvas.drawText(
					text,
					(CameraManager.get().getscreenResolution().x - textWidth) / 2 ,
