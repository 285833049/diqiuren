package com.earthman.app.nim.team;
...
public class TeamCreateHelper {
	 ...
	 * 创建讨论组
	 */
	public static void createNormalTeam(final Context context, String groupName, List<String> memberAccounts, final boolean isNeedBack, final RequestCallback<Void> callback) {
		...
		fields.put(TeamFieldEnum.Name, groupName);
		NIMClient.getService(TeamService.class).createTeam(fields, TeamTypeEnum.Normal, "", memberAccounts).setCallback(new RequestCallback<Team>() {
			@Override
			public void onSuccess(Team team) {
				DialogMaker.dismissProgressDialog();
				Toast.makeText(NimCache.getContext(), R.string.create_team_success, Toast.LENGTH_SHORT).show();
				if (isNeedBack) {
			...
			public void onFailed(int code) {
				...
				if (code == 801) {
					...
					Toast.makeText(NimCache.getContext(), tip, Toast.LENGTH_SHORT).show();
				} else {
					Toast.makeText(NimCache.getContext(), R.string.create_team_failed, Toast.LENGTH_SHORT).show();
				}
	 ...
	 * 创建高级群
	 */
	public static void createAdvancedTeam(final Context context, String teamName, List<String> memberAccounts) {
		...
		NIMClient.getService(TeamService.class).createTeam(fields, type, "", memberAccounts).setCallback(new RequestCallback<Team>() {
			@Override
			...
			public void onFailed(int code) {
				...
				String tip;
				if (code == 801) {
					tip = context.getString(R.string.over_team_member_capacity, DEFAULT_TEAM_CAPACITY);
				} else if (code == 806) {
	...
	private static void onCreateSuccess(final Context context, final Team team) {
		...
		Map<String, Object> content = new HashMap<String, Object>(1);
