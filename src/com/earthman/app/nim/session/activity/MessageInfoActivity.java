...
public class MessageInfoActivity extends TActionBarActivity {
	...
	private void createTeamMsg() {
		ArrayList<String> memberAccounts = new ArrayList<String>();
		...
		if (resultCode == Activity.RESULT_OK) {
			if (requestCode == REQUEST_CODE_NORMAL) {
				...
				if (selected != null && !selected.isEmpty()) {
					final DialogEditText dialogEditText = new DialogEditText(MessageInfoActivity.this, "请输入讨论组名称", "","",
							new EditTextDialogListener() {
								@Override
								public void onRightButtonClick(EditText etContent) {
									TeamCreateHelper.createNormalTeam(MessageInfoActivity.this, etContent.getText().toString(), selected, true, new RequestCallback<Void>() {
										@Override
										public void onSuccess(Void param) {
											finish();
										}
