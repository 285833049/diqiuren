package com.earthman.app.nim.uikit.common.ui.imageview;
 ...
 */
public class NImImageView extends ImageView {
	...
	private static final DisplayImageOptions createImageOptions() {
		...
		return new DisplayImageOptions.Builder().showImageOnLoading(defaultIcon).showImageOnFail(defaultIcon).cacheInMemory(true).cacheOnDisk(true)
	...
	public NImImageView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}

	public NImImageView(Context context, AttributeSet attrs, int defStyle) {
	...
	private void loadBuddyAvatar(final String account, final int thumbSize) {
		...
		if (needLoad) {
			 ...
			 */
			final String thumbUrl = makeAvatarThumbNosUrl(userInfo.getAvatar(), 1024);

			// 异步从cache or NOS加载图片
			ImageLoader.getInstance().displayImage(thumbUrl, new NonViewAware(new ImageSize(1024, 1024), ViewScaleType.CROP), options,
	...
	private static String makeAvatarThumbNosUrl(final String url, final int thumbSize) {
		return thumbSize > 0 ? NosThumbImageUtil.makeImageThumbUrl(url, NosThumbParam.ThumbType.Crop, thumbSize, 1024) : url;
	}
