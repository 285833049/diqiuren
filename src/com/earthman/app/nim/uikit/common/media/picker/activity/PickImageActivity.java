...
public class PickImageActivity extends Activity {
	...
	public static void start(Activity activity, int requestCode, int from, String outPath, boolean mutiSelectMode, int multiSelectLimitSize, boolean isSupportOrig, boolean crop,
	...
	private void pickFromLocal() {
		...
		try {
			...
			Toast.makeText(this, R.string.gallery_invalid, Toast.LENGTH_LONG).show();
			finish();
		} catch (Exception e) {
	...
	private Intent makeLaunchIntent(Context context, boolean mutiSelectMode, int mutiSelectLimitSize, boolean isSupportOrig) {
	...
	private String pathFromResult(Intent data) {
		...
		if (cursor == null) {
			...
			if (uri.toString().contains("content://com.android.providers.media.documents/document/image")) { // htc
																												// 某些手机
				// 获取图片地址
				String _id = null;
				...
				Cursor mcursor = getContentResolver().query(Images.Media.EXTERNAL_CONTENT_URI, null, " _id = " + _id, null, null);
				mcursor.moveToFirst();
