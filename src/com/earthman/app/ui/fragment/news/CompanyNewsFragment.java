...
/**
 * 作者：Zhou
 * 日期：2016-2-26 下午2:32:35
 * 描述：（）
 */
public class CompanyNewsFragment extends BaseFragment implements IXListViewListener, OnItemClickListener {
	...
	private void doGetHomeNews() {
		...
		list.add(pageNo);
		String getNewsUrl = HttpUrlConstants.getUrl(activity, HttpUrlConstants.GET_COMPANY_NEWS, list);
		executeRequest(new FastJsonRequest<GetHomeNewsResponse>(Method.GET, getNewsUrl, GetHomeNewsResponse.class, new Response.Listener<GetHomeNewsResponse>() {
			...
			public void onResponse(GetHomeNewsResponse response) {
				if ("000000".equals(response.getCode())) {
					...
					if (result != null) {
						updateNews(result.getCompanyNews());
