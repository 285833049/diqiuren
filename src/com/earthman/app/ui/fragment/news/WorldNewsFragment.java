...
/**
 * 作者：Zhou
 * 日期：2016-2-26 下午2:33:35
 * 描述：（）
 */
public class WorldNewsFragment extends BaseFragment implements IXListViewListener, OnItemClickListener{
	...
	private  void doGetHomeNews(){
		...
		list.add(pageNo);
		showLoading();
		String getNewsUrl = HttpUrlConstants.getUrl(activity, HttpUrlConstants.GET_WORLD_NEWS, list);
				...
				new Response.Listener<GetHomeNewsResponse>() {
					...
					public void onResponse(GetHomeNewsResponse response) {
						if ("000000".equals(response.getCode())) {
							...
							if(result != null){
								updateNews(result.getWorldsNews());
