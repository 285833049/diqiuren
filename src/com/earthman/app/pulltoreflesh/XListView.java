...
public class XListView extends ListView implements OnScrollListener {
	 ...
	 * @param view
	 */
	public void addExtraHead(View view){
		mHeader.addView(view);
		//mHeader.setVisibleHeight(view.getMeasuredHeight());
		invalidate();
	}
		...
		if (MotionEvent.ACTION_UP == event.getAction() && !dte) {// onItemClick
			try {
				int position = pointToPosition((int) event.getX(),
						(int) event.getY());
				if (getAdapter().getCount() < position) {
					return false;
				}
				View view = /*getChildAt(position)*/getAdapter().getView(position, null, null);
				if (view == null) {
					return false;
				}
